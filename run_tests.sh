...
# Source the centralized environment setup script
if [ -f ./setup_acpp_env.sh ]; then
    source ./setup_acpp_env.sh
else
    echo "WARNING: setup_acpp_env.sh not found. Environment may be incomplete."
fi
...
export ACPP_DEBUG_LEVEL=${ACPP_DEBUG_LEVEL:-3} # Default to 3 for verbose backend info
export OMP_NUM_THREADS=1
echo "--- AdaptiveCpp Debug Logging Enabled (Level ${ACPP_DEBUG_LEVEL}) ---"
echo "--- Look for '[ACPP] Runtime: Backend CUDA is active.' in the logs below ---"
